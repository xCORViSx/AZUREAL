...
- `src/app/session_cache.rs` - `CachedSession`, `CacheIndex`, `resolve_cache_name()`, `lookup_cache_name()`, `read_cache()`, `read_cache_orphan()`, `write_cache()`, `migrate_legacy_caches()`
...
│   │   ├── session_cache.rs # Intermediate session cache — gzip-compressed CachedSession with CacheIndex sequential naming (claude-N/codex-N), migrate_legacy_caches() auto-migration to .azureal/sessions/*.json.gz (pub(crate))
