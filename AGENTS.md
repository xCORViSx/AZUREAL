...
Sessions are stored in `.azureal/sessions.azs` — a single SQLite database (WAL mode) with S-numbered sessions (S1, S2, S3...). Backend-agnostic: a single session can span Claude and Codex prompts. The `.azs` extension discourages users from tampering with the binary file. Schema: `sessions` (id, name, worktree, created), `events` (session_id, seq, kind, data, char_len), `compactions` (session_id, after_seq, summary), `meta` (key, value). Store opened on project load and project switch; `current_session_id` tracks the active session. `pid_session_target` maps PIDs to `(session_id, worktree_path)` tuples at spawn time so results write to the correct session even after switching worktrees. Legacy JSONL sessions (UUID-keyed) coexist — `load_session_output()` routes by ID format (numeric → store, UUID → legacy JSONL). **Context injection replaces `--resume`:** When `current_session_id` is set, the prompt flow builds a `ContextPayload` from the store (`build_context()`), wraps it in `<azureal-session-context>` tags via `build_context_prompt()`, and spawns the agent with `resume_id = None`. The UI sees only the clean user prompt (`add_user_message(input)` is called before injection). Empty sessions (first prompt) pass the prompt unchanged. RCR sessions keep `--resume` (separate worktree context). Staged prompts follow the same injection path. **Post-exit flow:** When a Claude process exits and `pid_session_target` has an entry for its PID, `store_append_from_jsonl()` parses the JSONL session file, strips any injected `<azureal-session-context>` prefix from UserMessage events, and appends all DisplayEvents to the SQLite store. Background project exits use `store_append_background()` which opens a temporary store connection to the target project's `.azs` file. **Compaction:** After appending events, `store_append_from_jsonl()` checks `total_chars_since_compaction()` against the 400K character threshold (~100K tokens). When exceeded, `compaction_needed` is set and a background Haiku agent is spawned on the next agent exit event via `spawn_compaction_agent()`. The compaction agent receives a `build_compaction_prompt()` transcript and produces a 2000-4000 character summary. Compaction runs in isolated `compaction_receivers`/`compaction_output` maps (invisible to UI — no banners, no slot registration). On completion, `poll_compaction_agents()` stores the summary via `store_compaction()` and subsequent `build_context()` calls include it as a prefix. **Legacy migration:** On first store open (per project), `migrate_from_legacy()` reads `.azureal/sessions/index.json` and imports all `.json.gz` cache files into the SQLite store. Each cached session's events are appended to a new S-numbered session with the custom display name (or cache name like "claude-1"). A UUID→worktree map built from `list_sessions()` assigns migrated sessions to the correct worktree. Idempotent via `legacy_migrated` meta key. `get_meta()`/`set_meta()` helpers provide general-purpose key-value storage in the meta table.
...
Implementation: `src/tui/event_loop.rs` + `src/tui/event_loop/` (7 submodules: actions, claude_events, claude_processor, coords, fast_draw, input_thread, mouse) for event loop, `src/tui/run.rs` for rendering, `src/tui/render_thread.rs` for background session rendering, `src/app/state/` for state management (split into 10 focused submodules, `health` has 2 sub-submodules). `actions` itself is split into 6 sub-submodules: execute, navigation, escape, session_list, deferred, rcr.
...
- **Extended typing deferral (300ms window):** `typing_recently` (true when `last_key_time.elapsed() < 300ms`) suppresses `terminal.draw()` calls during active typing when fast-path is available. `fast_draw_input()` (~0.1ms) provides instant keystroke feedback, and `fast_draw_session()` (~2-5ms via direct cursor positioning) keeps the session pane updating during the deferral window. Event loop profiler logs to `~/.azureal/event_loop_profile.log` for diagnostics.
...
All session data lives in `.azureal/sessions.azs` — a single SQLite database (WAL mode) with S-numbered sessions (S1, S2, S3...). On prompt send, `build_context()` reconstructs prior conversation from stored events and injects it as `<azureal-session-context>` tags (replacing `--resume`). On Claude exit, the JSONL file is parsed, context tags stripped, and events appended to the store. Compaction runs automatically at 400K chars (~100K tokens) via a background Haiku agent that summarizes the conversation — subsequent context injections use the compact summary as a prefix. Legacy `.json.gz` caches are auto-migrated on first open via `migrate_from_legacy()`.
...
- `src/app/session_store.rs` - `SessionStore`, `SessionInfo`, `CompactionInfo`, `ContextPayload`, `MigrationResult`, `open()`, `create_session()`, `rename_session()`, `delete_session()`, `list_sessions()`, `append_events()`, `load_events()`, `build_context()`, `store_compaction()`, `migrate_from_legacy()`, `parse_legacy_index()`, `read_legacy_cache()`
...
- **Fast-path input rendering:** When keys arrive in Claude prompt mode (NOT terminal mode) with **single-line input** (no `\n`) and **no active selection**, `fast_draw_input()` writes the input box content directly to the terminal via crossterm (~0.1ms), completely bypassing `terminal.draw()` (~18ms). **Runs immediately after the event drain loop** — before Claude event processing, file watcher drain, file tree rebuild, worktree refresh, render submit, or any other housekeeping. This ordering is critical: blocking operations (file tree rebuild ~10-100ms, worktree refresh ~10-50ms) fire every 500ms when Claude modifies files; if fast_draw ran after them, keystrokes would visually lag by 20-150ms. The full `terminal.draw()` is deferred to the next quiet frame. Ratatui's diff naturally reconciles on the next full draw (no buffer invalidation needed). `app.input_area` (cached from last full draw in `ui()`) provides the screen coordinates. **Must exclude terminal mode** — terminal uses `prompt_mode=true` for "type mode", but `fast_draw_input()` writes `app.input` (empty in terminal) over the input_area, wiping PTY display. **Must exclude multi-line input** — the input box resizes dynamically when newlines are added/removed, but `input_area` reflects the old height, causing cursor mispositioning. **Must exclude active selection** — `fast_draw_input()` writes raw text without selection highlighting; `has_input_selection()` check added to both fast-path and draw deferral conditions.
- **Fast-path session rendering (direct cell writes):** When Claude is streaming, the user is in follow-bottom mode, and new incremental lines arrive from the render thread, `fast_draw_session()` updates the session pane without a full `terminal.draw()`. Rewrites all visible session lines in the session pane column via direct cursor positioning (~10-15KB for the session column only vs ~87KB for ratatui's full terminal diff). Does NOT use DECSTBM scroll regions — those are full-width (scroll all columns on affected rows) and would blank the file tree and viewer panes. Color conversion from ratatui's `Color` enum to crossterm's `Color` enum is handled by `ratatui_to_crossterm()` (maps all 16 ANSI colors, RGB, and 256-color indexed). Combined with `fast_draw_input()`, both input and session content update simultaneously during streaming.
- **Extended typing deferral (300ms window):** When typing single-line in Claude prompt mode with no selection, `terminal.draw()` is SKIPPED for the entire 300ms `typing_recently` window — not just the single key frame. `fast_draw_input()` (~0.1ms) and `fast_draw_session()` (~2-5ms direct cell writes) provide visual feedback during the deferral. Events are always applied and redraws always triggered (no `suppress_redraw`); render results are always polled immediately (no `defer_render_poll`). Only the expensive full `terminal.draw()` is deferred. A `draw_pending` flag on App tracks deferred draws. **Terminal type mode, multi-line input, and active selection are NOT deferred** — they need immediate `terminal.draw()` calls (PTY has no fast-path; multi-line needs layout resize; selection needs full render for highlight styling).
- **Overlay panel guards for fast_draw:** `fast_draw_session()` bypasses ratatui's buffer and writes directly to the terminal via escape sequences. Any full-screen overlay (projects panel, git panel, session list) must be guarded — without the check, session content bleeds through the overlay. Guards: `app.git_actions_panel.is_none()`, `!app.show_session_list`, `!app.is_projects_panel_active()`. The projects panel also renders `Clear` on the full `f.area()` (not just the modal rect) so ratatui's diff rendering overwrites all old cells.
- **Force full redraw on layout switch:** `fast_draw_session()` writes directly to the terminal, making ratatui's internal buffer stale for those cells. When the layout changes (e.g. git panel open/close), ratatui's diff misses cells it doesn't know were changed, leaving visual fragments. `force_full_redraw: bool` on App is set by `open_git_actions_panel()` and `close_git_actions_panel()`. The event loop checks this flag before `terminal.draw()` and calls `terminal.clear()` first to reset ratatui's buffer, forcing a complete redraw.
...
- **Immediate render poll + fast_draw_session:** Render results are always polled immediately — no deferral during typing. When new incremental lines arrive during streaming + follow-bottom mode, `fast_draw_session()` rewrites the visible session pane via direct cursor positioning (~2-5ms) instead of waiting for the next full `terminal.draw()`. This replaced the old `defer_render_poll` / `suppress_redraw` approach: previously, render polls and redraw flags were suppressed during typing+streaming to keep ratatui diffs small, but this froze the session pane during typing. Now events are always applied, the session pane always updates (via fast-draw), and only the expensive full `terminal.draw()` is deferred during the typing window.
...
**Files:**
- `src/tui/run.rs` — `draw_worktree_tabs()` renders the tab row with pagination
...
**Affected:** `fast_draw_session()` — originally used DECSTBM, now uses direct cell writes. DECSLRM (left/right margins) exists but is not widely supported (Terminal.app doesn't support it).
...
Implementation: `draw_worktree_tabs()` in `src/tui/run.rs`, `worktree_tab_hits: Vec<(u16, u16, Option<usize>)>` in `src/app/state/app.rs`, mouse click handling in `src/tui/event_loop/mouse.rs`
...
- **Actions section** (sidebar top, auto-height): Context-aware git operations with single-key shortcuts, navigable with j/k. A `─` divider separates actions from toggles (auto-rebase, auto-resolve). Height: main=8 rows, feature=10 rows. Focus: `panel.focused_pane == 0`.
...
Implementation: `src/app/state/load.rs` (`dump_debug_output()`), `src/tui/run.rs` (`draw_debug_dump_naming()`, `draw_debug_dump_saving()`), `src/tui/event_loop/actions.rs` (dialog handler), `src/tui/event_loop/actions/execute.rs` (`Action::DumpDebug`), `src/tui/event_loop.rs` (deferred saving)
...
Two name sources merged by `load_all_session_names()`:
- **SQLite store** — S-numbered sessions. Name defaults to "S{id}" (e.g. "S1", "S2"), overridden by `rename_session()`. Keyed by integer ID as string.
- **Legacy cache index** — `.azureal/sessions/index.json` maps UUID → `{"cache": "claude-1", "name": "hook-visibility-fix"}`. Keyed by UUID string.

No collisions: integer strings ("1", "2") never match UUID format. Display name fallback chain: custom name → S-number/cache filename → UUID short format.

Implementation: `src/wizard.rs` (wizard state), `src/tui/draw_wizard.rs` (rendering), `src/tui/input_wizard.rs` (input handling), `src/app/state/session_names.rs` (name storage — merges legacy cache + SQLite store)
...
│   ├── sessions.azs        # SQLite session store (.azs = obscure extension, internally standard SQLite with WAL mode) — portable sessions with S-numbering
│   └── sessions/           # Legacy session cache dir (gitignored) — generated .json.gz files, being replaced by sessions.azs
...
│   │   │   ├── load.rs     # Session loading and discovery; main stored in main_worktree (not worktrees vec). Dual-path: loads from SQLite store (numeric IDs) or legacy JSONL (UUID IDs)
...
│   │   │   ├── project_snapshot.rs # Per-project state snapshot for parallel switching — pid_session_target stores (session_id, worktree_path) tuples, current_session_id
...
│   │   ├── session_store.rs # SQLite-backed session store (.azs) — SessionStore wrapping rusqlite::Connection with WAL mode, S-numbered sessions, event append/load, compaction support, context building for session resume. Tables: sessions, events, compactions, meta. Key types: SessionInfo, CompactionInfo, ContextPayload (pub(crate))
...
│   │   ├── session_cache.rs # REMOVED (Phase 9 cleanup) — replaced by session_store.rs
...
│   │   ├── run.rs          # TUI entry point, splash screen, 3-pane layout (normal + git mode branches), draw_git_status_box()
...
│   │   │   ├── agent_events.rs # Agent process event handling + staged prompt (Claude + Codex dispatch), compaction agent spawning (spawn_compaction_agent), compaction polling (poll_compaction_agents), streaming JSON assistant text extraction (extract_assistant_text)
...
## Phase 4: Portable Sessions (In Progress)
- [x] SQLite-backed session store (`.azureal/sessions.azs`) with WAL mode, S-numbered sessions
...
- [x] Compaction system (400K char threshold, background Haiku agent summarization)
- [x] Migration from legacy `.json.gz` caches to SQLite store
- [x] Cleanup (remove session_cache.rs, old polling fields, cache read/write paths)
...
| tui/render_tools | `src/tui/render_tools.rs` | 99 | `tool_display_name` (12 -- Grep/Glob rename, Read/Write/Bash/Edit/Task/WebFetch passthrough, empty, unknown), `extract_tool_param` (42 -- Read/Write/Edit file_path+path fallback+empty+null, Bash command+long, Glob/Grep pattern, WebFetch url, WebSearch query, Task subagent_type+default+empty, LSP operation+file+empty, EnterPlanMode/ExitPlanMode, unknown tool fallback priority chain), `truncate_line` (14 -- exact-fit, under-max, over-max, max-1/max-0, empty, whitespace-trim, unicode, emoji, special chars), `render_tool_result` (23 -- Read empty/single/two/many lines, Bash single/multi/empty-lines/all-empty, Grep few/exact-3/more-than-3, Glob count, Task few/many, default few/many, failed-red/success-azure, system-reminder stripping, narrow-width truncation), `render_write_preview` (10 -- content with lines, comment detection for `//`/`#`/`/*`/`///`/`//!`, no-comment first-line fallback, no-content-field, empty content, checkmark) |
...
| session_store | `src/app/session_store.rs` | 80 | `SessionStore::open` (WAL mode, schema creation, idempotent), `create_session` (auto-increment, worktree assignment), `rename_session`/`delete_session` (CRUD), `list_sessions` (filtered/unfiltered, event+message counts via JOIN), `load_all_session_names` (S-number default, custom name), `append_events` (seq numbering, Filtered skipping, char_len, transaction), `load_events`/`load_events_from` (ordering, JSON round-trip), `count_events` (kind filtering), `message_count` (UserMessage+AssistantText), `total_chars_since_compaction` (with/without compaction), `store_compaction`/`latest_compaction`, `max_seq`, `build_context` (full/partial/empty), `event_kind` (all DisplayEvent variants), `event_char_len` (content extraction), `db_path`, `get_meta`/`set_meta` (missing-returns-none, set-then-get, overwrites, schema_version), `parse_legacy_index` (current-format, bare-string, empty, invalid-json, mixed-values), `read_legacy_cache` (valid-gzip, missing-file, corrupt-gzip, no-events-field), `migrate_from_legacy` (no-index, empty-index, single-session, multiple-sessions, idempotent, skips-missing-cache, skips-empty-cache-name, uses-cache-name-fallback, worktree-mapping, unknown-uuid-empty-worktree, legacy-bare-string-index, skips-empty-events, filtered-events-skipped, result-fields, nonexistent-sessions-dir) |
| context_injection | `src/app/context_injection.rs` | 37 | `build_context_prompt` (empty events no-op, with events wraps in tags, with compaction summary prefix, preserves user prompt), `strip_injected_context` (returns clean prompt, no tags returns original, handles missing close tag), `build_transcript` (formats UserMessage/AssistantText/ToolCall/ToolResult/Plan, skips Init/Hook/Complete/Filtered/Compacting/Compacted/MayBeCompacting/Command, pairs ToolCall+ToolResult), `format_event` (all variant coverage), `extract_key_param` (Read/Write/Edit/Bash/Glob/Grep/Task/WebFetch/WebSearch/LSP), `compact_result` (truncation at 500 chars), `build_compaction_prompt` (contains transcript tags, includes instruction keywords, empty payload valid, includes compaction summary) |
...
| tui/event_loop/actions/session_list | `src/tui/event_loop/actions/session_list.rs` | 54 | Session list open (SQLite store + legacy JSONL merge), finish load (skips store sessions), JSONL message counting, navigation, selection, filtering |
...
| tui/input_git_actions/operations | `src/tui/input_git_actions/operations.rs` | 56 | `parse_conflict_files` (5 -- mixed/auto-merged-only/whitespace-trimming), auto-resolve boundary checks, overlay field construction (GitConflictOverlay/GitCommitOverlay/GitChangedFile/PostMergeDialog), commit message markdown fence stripping, push note formatting |
