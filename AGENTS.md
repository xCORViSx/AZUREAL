...
- **Secondary key drain after Claude events:** After processing Claude streaming events (JSON parse, up to 10/tick), a second `event::poll(0)` drain loop catches keys that arrived during the 1-10ms of parsing. Without this, those keys wait until the next iteration's primary drain — delayed by housekeeping + draw. Only runs during active streaming when Claude events were actually processed. Caught keys get `fast_draw_input()` for immediate visual feedback.
