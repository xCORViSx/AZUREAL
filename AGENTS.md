...
- **Fast-path input rendering:** When keys arrive in prompt mode, `fast_draw_input()` writes the input box content directly to the terminal via crossterm (~0.1ms), completely bypassing `terminal.draw()` (~18ms). The key is processed immediately and the input character appears instantly. The full `terminal.draw()` is deferred to the next quiet frame. Ratatui's diff naturally reconciles on the next full draw (no buffer invalidation needed). `app.input_area` (cached from last full draw in `ui()`) provides the screen coordinates.
- **Deferred draw on key events:** When keys arrive, `terminal.draw()` is SKIPPED — the key is processed, fast-path renders the input, and the loop iterates back immediately. Full draw happens on the next quiet iteration (no key events). A `draw_pending` flag on App tracks deferred draws.
