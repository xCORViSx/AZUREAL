...
7. **Leader entry:** Plain `w` key (no modifiers, not in prompt/terminal/edit mode) sets `leader_state = WaitingForAction` and shows `[w …]` in the status bar. Fires after modal dispatch but before focus-specific handlers
...
Destructive worktree actions are behind a two-key leader sequence: press `w`, then the action key. The status bar shows `[w …]` while waiting. Press `Esc` to cancel. State tracked via `LeaderState` enum on App (`None` / `WaitingForAction`).
