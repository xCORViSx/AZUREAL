...
- **Extended typing deferral (300ms window):** `typing_recently` (true when `last_key_time.elapsed() < 300ms`) suppresses `terminal.draw()` calls during active typing when fast-path is available. Both `fast_draw_input()` (~0.1ms) and `fast_draw_session()` (~0.1ms per new line via DECSTBM scroll regions) continue to provide instant visual feedback during the deferral window — input keystrokes AND session pane streaming content update simultaneously without any full terminal draw. **Profiler-confirmed root cause:** single-frame deferral (`had_key_event`) only skipped 1 of 368 draws during a streaming+typing session — Terminal.app delays keyboard forwarding while processing escape sequences (~6ms per draw), causing ~10% keystroke timing distortion (repeated/garbled characters). The 300ms window eliminates ALL draws during active typing (368 draws/session → 0). **Profiler-confirmed results:** error rate dropped from 10.4% to 3.2% (remaining errors are finger timing, not app-caused), longest perfect character alternation went from ~20 chars to 381 chars. Event loop profiler logs to `~/.azureal/event_loop_profile.log` for future diagnostics.
...
- **Fast-path session rendering (DECSTBM scroll regions):** When Claude is streaming, the user is in follow-bottom mode, and new incremental lines arrive from the render thread, `fast_draw_session()` updates the session pane without a full `terminal.draw()`. Uses terminal scroll regions (`DECSTBM` — `\x1b[top;bottom r`) to set a scroll region on the session pane content area, scrolls existing content up via `ScrollUp(n)`, and writes only the new line(s) at the bottom. ~200 bytes per new line vs ~87KB for ratatui's full cell-by-cell diff when content scrolls. Resets scroll region to full terminal after writing. Color conversion from ratatui's `Color` enum to crossterm's `Color` enum is handled by `ratatui_to_crossterm()` (maps all 16 ANSI colors, RGB, and 256-color indexed). Combined with `fast_draw_input()`, both input and session content update simultaneously during typing+streaming — the session pane freeze during active typing is eliminated.
- **Extended typing deferral (300ms window):** When typing single-line in Claude prompt mode with no selection, `terminal.draw()` is SKIPPED for the entire 300ms `typing_recently` window — not just the single key frame. Both `fast_draw_input()` (~0.1ms) and `fast_draw_session()` (~0.1ms per new line via DECSTBM scroll regions) provide instant visual feedback during the deferral — input keystrokes AND session pane streaming content update simultaneously. Events are always applied and redraws always triggered (no `suppress_redraw`); render results are always polled immediately (no `defer_render_poll`). Only the expensive full `terminal.draw()` is deferred. A `draw_pending` flag on App tracks deferred draws. **Terminal type mode, multi-line input, and active selection are NOT deferred** — they need immediate `terminal.draw()` calls (PTY has no fast-path; multi-line needs layout resize; selection needs full render for highlight styling).
...
│   │   │   ├── fast_draw.rs # Fast-path input + session rendering (~0.1ms bypass via DECSTBM scroll regions)
...
| tui/event_loop/fast_draw | `src/tui/event_loop/fast_draw.rs` | 53 | `word_wrap_break_points` (newline/long-line/multiple/width-1), `display_width` (single/CJK/mixed), scroll offset edge cases, padded text, adjusted row underflow, Rect coordinate math, unicode_width fallback, input_area defaults, cursor row assignment, `ratatui_to_crossterm` color mapping (ANSI/RGB/indexed), `fast_draw_session` scroll region output |
