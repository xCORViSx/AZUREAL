...
Parsed session data is written to `.azureal/sessions/<backend-N>.json.gz` (e.g. `claude-1.json.gz`, `codex-2.json.gz`) as a gzip-compressed unified intermediate format. An `index.json` maps session UUIDs to sequential cache names; counters are independent per backend and continue incrementing even when sessions are user-renamed. On subsequent loads, the cache is checked first (validated by `source_path` + `source_size` match) — if valid, the TUI hydrates directly from the cache, skipping raw JSONL parsing entirely. Cache misses trigger raw parse → cache write → hydrate. Incremental polls also rewrite the cache after merging new events. When the raw session file is missing but a cache exists, the session is loaded in **read-only mode**: the session pane shows the cached conversation, but the input box displays "SESSION FILE MISSING @ `<path>`" in bold red and blocks prompt submission. Recovery is automatic — if the source file reappears, `check_session_file()` clears the flag and re-enables input.

**Cache compaction** minimizes file size via three strategies: (1) `#[serde(skip)]` on internal-only `_uuid`/`_session_id`/`_message_id` fields (never read by render pipeline), (2) `#[serde(skip_serializing_if)]` omits default values (empty sets, None, false, zero), (3) `compact_events()` truncates `ToolResult.content` to match render_tools.rs display rules (Read: first+last line, Bash: last 2 non-empty, Grep: first 3, Glob: count only, Task: first 5, default: first 3) and strips `ToolCall.input` to display-relevant fields only (Write input.content → line count + purpose line summary; Edit preserved fully for diff rendering; all others retain only the key field `extract_tool_param` reads). Applied in both `from_parsed()` and the direct `CachedSession` construction path in `refresh_session_events()`. Cache files are then gzip-compressed via `flate2` (`Compression::fast()`) for an additional 3-10x size reduction; decompression is transparent on read.
...
- `src/app/session_cache.rs` - `CachedSession`, `CacheIndex`, `IndexEntry`, `resolve_cache_name()`, `lookup_cache_name()`, `read_cache()`, `read_cache_orphan()`, `write_cache()`, `backend_from_path()`, `save_session_name()`, `load_all_session_names()`
...
- "⏳ Session may be compacting conversation..." (yellow, `MayBeCompacting` variant) - shown when context usage ≥ 90% AND 20 seconds pass with no new events parsed from the session. Since there's no way to detect auto-compaction until after it completes, this inactivity heuristic warns the user why the session pane appears frozen. Banner is injected once per high-context period; cleared when new events arrive or context drops below 90%.
...
**Compaction inactivity watcher:** When context usage ≥ 90%, `update_token_badge()` sets `context_pct_high = true`. The event loop checks: if `context_pct_high && !compaction_banner_injected && agent_receivers non-empty && last_session_event_time elapsed ≥ 20s`, it injects a `DisplayEvent::MayBeCompacting` banner. When new events arrive (in `handle_claude_output()` or `refresh_session_events()`), both `last_session_event_time` and `compaction_banner_injected` are reset. When context drops below 90% (e.g. after compaction completes), `compaction_banner_injected` is cleared.
...
Custom session names stored alongside cache index in `.azureal/sessions/index.json`:
```json
{
  "9d409dfb-422b-4f4b-9f32-755277e3e527": {"cache": "claude-1", "name": "hook-visibility-fix"},
...
│   └── sessions/           # Session cache dir (gitignored) — generated .json.gz files, regenerated from JSONL on load
...
│   │   │   ├── session_names.rs # Custom session name storage — delegates to session_cache::save_session_name/load_all_session_names (index.json)
...
│   │   ├── session_cache.rs # Intermediate session cache — gzip-compressed CachedSession with CacheIndex (IndexEntry: cache name + optional display name), sequential naming (claude-N/codex-N), backend_from_path() infers backend from session file path, session display names (save/load) — .azureal/sessions/*.json.gz + index.json (pub(crate))
...
| session_cache | `src/app/session_cache.rs` | 60 | `CachedSession::from_parsed` (2 -- preserves all fields, awaiting_plan_approval), `into_parsed` round-trip (1), serde round-trip (3 -- full/empty/all-event-variants), serde skip (2 -- underscore fields omitted from JSON with empty-string round-trip, default values omitted with round-trip restoration), `cache_path` (2 -- structure/uuid-special-chars), file I/O (7 -- write+read/rejects-wrong-size/rejects-wrong-source-path/missing-returns-none/orphan-ignores-validation/orphan-missing-returns-none/creates-directories/overwrites-existing), compaction — ToolResult.content (7 -- Read truncates-large/preserves-small, Bash keeps-last-two, Grep keeps-first-three, Glob shows-count, Task keeps-first-five, unknown keeps-first-three, strips system-reminder), compaction — ToolCall.input (4 -- Write summarizes-content, Edit preserved, Bash strips-extras, Read strips-extras), compaction integration (1 -- from_parsed compacts without mutating original), compression (3 -- gzip round-trip with magic bytes, compressed smaller than json, corrupted gzip returns None), CacheIndex sequential naming (13 -- resolve assigns claude-1/codex-1 first, increments per backend, returns existing for same UUID, lookup returns None for unknown/returns assigned name, next_number empty/skips-non-matching/handles-gaps, index persists across reads, index handles corrupt file, cache files use sequential names, mixed backends independent counters), session display names (6 -- save-and-load, overwrites, preserves-cache-entry, empty-index, skips-unnamed, legacy-index-format-reads-as-unnamed), integration (1 -- real session full pipeline: parse 19MB JSONL → compact → gzip → claude-1.json.gz → read back → verify index + round-trip), backend_from_path (3 -- claude/codex/unknown-falls-back), edge cases (4 -- tool-result-with-error/large-text-content/none-optionals) |
...
| app/state/session_names | `src/app/state/session_names.rs` | 50 | Session name resolution, UUID mapping |
