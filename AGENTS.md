...
| session_cache | `src/app/session_cache.rs` | 19 | `CachedSession::from_parsed` (2 -- preserves all fields, awaiting_plan_approval), `into_parsed` round-trip (1), serde round-trip (3 -- full/empty/all-event-variants), `cache_path` (2 -- structure/uuid-special-chars), file I/O (7 -- write+read/rejects-wrong-size/rejects-wrong-source-path/missing-returns-none/orphan-ignores-validation/orphan-missing-returns-none/creates-directories/overwrites-existing), edge cases (4 -- tool-result-with-error/large-text-content/none-optionals) |
