...
- **Deferred render poll during typing:** When the user is actively typing during Claude streaming (`last_key_time.elapsed() < 150ms` and `claude_receivers` non-empty), `poll_render_result()` is skipped. This prevents new session content from being applied to the cache right before a draw, which would increase the ratatui diff size and produce larger escape-sequence bursts that overwhelm the terminal emulator. The dirty flag stays set and fires on the next quiet poll. Has no effect on fast_draw_input (still runs immediately) or input processing.
- **Claude event cap per tick:** Raw Claude events are capped at 10 per tick (`MAX_CLAUDE_EVENTS_PER_TICK`). With background parsing, forwarding is just a channel send (~0.1ms each), so the cap is mostly a safety guard now.
