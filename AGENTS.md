...
**Auto-rebase:** Press `a` in the Git panel (feature branches only) to toggle auto-rebase for that worktree. Persisted in `.azureal/azufig.toml` `[git]` section as `auto-rebase/<branch> = "true"`. In-memory state: `App.auto_rebase_enabled: HashSet<String>` loaded from `azufig::load_auto_rebase_branches()` on startup. Every 2 seconds, `check_auto_rebase()` in `event_loop.rs` iterates enabled worktrees, skipping any with: Claude running (`is_session_running`), active RCR, git panel open, file being edited, or **dirty working tree** (`git status --porcelain` non-empty). Calls `exec_rebase_inner()` — on `Rebased`, shows a 2-second green success dialog (`auto_rebase_success_until`); on `Conflict`, switches to the worktree and opens the Git panel with conflict overlay (same RCR flow as manual rebase); on `UpToDate`/`Failed`, silent. Sidebar shows colored `R` indicator per worktree: green (auto-rebase on, idle), orange (RCR active), blue (RCR approval pending), none (disabled). `accept_rcr()` and abort both set `sidebar_dirty = true` to update the `R` color. Config keys cleaned up on worktree archive/delete via post-merge dialog. Orphaned rebase state (`.git/rebase-merge/`) and detached HEADs repaired inside `load_worktrees()` itself (not just startup) — any worktree with `branch: None` gets orphaned rebase aborted and HEAD re-attached via `git for-each-ref --points-at=HEAD` → `git checkout <branch>`, then the worktree list is re-fetched. This prevents blank sidebar entries and false archived duplicates after runtime operations that leave HEAD detached.
