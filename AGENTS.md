...
Implementation: `src/tui/event_loop.rs` + `src/tui/event_loop/` (6 submodules: actions, claude_events, coords, fast_draw, input_thread, mouse) for event loop, `src/tui/run.rs` for rendering, `src/tui/render_thread.rs` for background session rendering, `src/app/state/` for state management (split into 10 focused submodules, `health` has 2 sub-submodules). `actions` itself is split into 6 sub-submodules: execute, navigation, escape, session_list, deferred, rcr.
...
- **Claude event cap per tick:** During streaming, Claude sends ~60 events/sec. Draining all at once would block 10-50ms of sequential JSON parsing. Events are capped at 10 per tick (`MAX_CLAUDE_EVENTS_PER_TICK`); remaining stay in the channel for the next iteration. At 10 events × ~1ms each = 10ms max before returning to key polling.
- **Secondary drain after Claude events:** After processing Claude streaming events (JSON parse, up to 10/tick), drains the input channel for keys buffered during the 1-10ms of parsing. Only runs during active streaming when Claude events were actually processed. Caught keys get `fast_draw_input()` for immediate visual feedback.
- **Diagnostic key counters:** `diag_key_events` (all Press/Repeat received), `diag_key_repeats` (Repeat-only subset), and `diag_chars_inserted` (from `input_char()`) displayed as `K:N/R:N/C:N` in the status bar badge during prompt mode. Reset on `clear_input()`. If K > C, keys received but lost in dispatch; if R > 0, Kitty Repeat events are firing; if both K and C < expected, terminal-level drops.
- **Repeat filter for prompt mode:** `KeyEventKind::Repeat` events for `KeyCode::Char(_)` are silently discarded in prompt mode (not terminal mode). Prevents the Kitty protocol from inserting extra characters when keys briefly overlap during alternating typing. Repeat still accepted for navigation keys (arrows, backspace, delete) so holding them works.
