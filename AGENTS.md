...
- **Deferred draw on key events:** `terminal.draw()` costs ~18ms (measured). When keys arrive, the draw is SKIPPED — the key is processed immediately and the loop iterates back to read the next key. Draw happens on the next quiet iteration (no key events, ~16ms later). This means typing is NEVER blocked by draw. A `draw_pending` flag on App tracks when a draw is needed but deferred.
