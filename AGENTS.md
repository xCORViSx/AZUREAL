...
- `src/app/session_cache.rs` - `CachedSession`, `read_cache()`, `read_cache_orphan()`, `write_cache()`
...
│   │   ├── session_cache.rs # Intermediate session cache — gzip-compressed CachedSession read/write to .azureal/sessions/*.json.gz (pub(crate))
