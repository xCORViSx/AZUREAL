...
- **Diagnostic key counters:** `diag_key_events` (incremented on every Press/Repeat received in any drain loop) and `diag_chars_inserted` (incremented in `input_char()`) are displayed as `K:N/C:N` in the status bar badge during prompt mode. Reset on `clear_input()`. If K > C, keys are received but lost in dispatch; if both < expected, keys are lost at the terminal/crossterm layer.
