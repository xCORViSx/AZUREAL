...
- `src/app/session_store.rs` - `SessionStore`, `SessionInfo`, `CompactionInfo`, `ContextPayload`, `open()`, `create_session()`, `rename_session()`, `delete_session()`, `list_sessions()`, `append_events()`, `load_events()`, `load_events_range()`, `compaction_boundary()`, `build_context()`, `store_compaction()`
...
│   │   ├── session_store.rs # SQLite-backed session store (.azs) — SessionStore wrapping rusqlite::Connection with DELETE journal mode, S-numbered sessions, event append/load, boundary-based compaction (compaction_boundary + load_events_range), context building for session resume. Tables: sessions, events, compactions, meta. Key types: SessionInfo, CompactionInfo, ContextPayload (pub(crate))
...
- [x] Compaction system (400K char threshold, background agent summarization — Haiku/codex-mini per backend)
