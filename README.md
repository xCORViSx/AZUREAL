...
- **Git Panel** — `Shift+G` toggles a centered overlay with bold title, Git brand orange borders and brown secondary text; provides 5 git operations (rebase from main, merge to main, fetch, pull, push) with single-key shortcuts; merge runs from repo root so it works on any selected worktree without checkout; shows changed files with per-file color-coded `+N/-N` stats (green additions, red deletions); navigate files with `j/k`, press `Enter` to view a file's diff in the Viewer pane; Tab toggles between actions and file sections
