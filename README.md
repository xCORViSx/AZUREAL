...
- **Async Rendering** — Session pane renders on a background thread with backpressure + 50ms throttle; incremental renders send only new events (pre-scanned state from already-rendered events avoids mega-clones); single JSON parse per streaming event; dedicated input reader thread ensures keystrokes are captured even during screen draws; input is never blocked by markdown/syntax processing
