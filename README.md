...
- **Git Panel** — `Shift+G` toggles a centered overlay with bold title, Git brand orange borders and brown secondary text; provides 5 git operations (rebase, merge, fetch, pull, push) with single-key shortcuts; shows changed files with per-file color-coded `+N/-N` stats (green additions, red deletions); navigate files with `j/k`, press `Enter` to view a file's diff in the Viewer pane; Tab toggles between actions and file sections; `H` toggles `.git` directory visibility in file tree (hidden by default, shown as ✓/✗ indicator on top-right border)
