...
- Process killing (`src/backend.rs`, `src/app/state/ui.rs`): `kill_process_tree(pid)` kills the entire process group. On Unix, agents are spawned with `process_group(0)` (own process group leader), killed via `libc::killpg()` so all children (cargo test, subagents, etc.) die too. On Windows, `taskkill /PID /T /F` (tree kill). `cancel_all_claude()` kills all agent + compaction processes and is called on app quit. Claude subprocess spawned with `.stdin(Stdio::null())` to prevent console stdin handle sharing on Windows (causes input event competition between TUI and child).
