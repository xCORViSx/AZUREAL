...
- Paste coalescing (`src/tui/event_loop.rs`): On Windows, crossterm's `ReadConsoleInputW`-based reader delivers pasted text as individual `KEY_EVENT` records — `Event::Paste` is never generated despite `EnableBracketedPaste`. The event loop's drain collects all events into a batch and passes them through `coalesce_paste_events()` which detects paste patterns: ≥3 char/Enter Press events with characters appearing after an Enter (impossible via human typing — Enter submits the prompt before subsequent characters can arrive in the same drain cycle). Detected pastes are coalesced into a single `Event::Paste(text)` which routes through the existing per-mode paste handlers. Drain extension: when a batch has ≥3 key presses OR ends with Enter, the loop waits up to 5ms for more events (paste events trickle across drain boundaries on Windows). **Paste guard safety net:** Even with drain extension, paste events can span drain cycles — if Enter submits mid-paste, remaining chars are processed with `prompt_mode=false`. `paste_guard_until` (20ms `Instant` on App, set on prompt submit) suppresses single-letter globals via `KeyContext.paste_guard` in `lookup_action()` skip conditions. This prevents paste remnants like `Shift+P` from triggering `OpenProjects`.
