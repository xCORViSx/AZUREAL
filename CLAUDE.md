...
- **Branch prefix migration:** On startup and project switch, `Git::migrate_branch_prefix()` scans worktree and archived branches — any branch whose prefix doesn't match the project's expected prefix is renamed (local + remote push + old remote ref deleted). E.g. `azureal/clips` → `idiosonix/clips` when the project's repo is `iDiOSONiX`. Status bar shows "Migrated N branch(es) to prefix/" on first load.
...
│   │   ├── branch.rs       # Branch management (list_local_branches, list_remote_branches_cached, get_main_branch, get_current_branch, migrate_branch_prefix)
