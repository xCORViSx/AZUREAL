...
- Git branches via `git branch | grep {branch_prefix}/` for archived worktrees (prefix derived from git remote origin repo name, falling back to folder name — via `Project.branch_prefix`, computed by `branch_prefix_for_path()` in `src/models.rs`)
...
- **Global** `~/.azureal/azufig.toml` — app config (API key, claude path, permission mode, startup screen toggle), registered projects (paths + display names), global run commands, global preset prompts
- **Project-local** `.azureal/azufig.toml` — filetree options (hidden entry names), health scan scope (directory paths), project-local run commands, project-local preset prompts. Always at the **main worktree root** (resolved via `git rev-parse --git-common-dir` parent), shared by all worktrees — no per-worktree copies. **Gitignored by default** — `ensure_worktrees_gitignored()` respects existing `.gitignore` entries and otherwise writes `.azureal/` and `worktrees/` to Git's local `info/exclude`, avoiding startup commits or index mutations while preventing runtime files from causing rebase conflicts.
- **Worktree-local** `.azureal/azufig.toml` — git settings (auto-rebase toggle, auto-resolve file list). At each **worktree path** (`worktrees/<name>/`), so these are gitignored and local-only. `auto-rebase = "true"` is a simple boolean (no branch prefix — the file IS the worktree). Auto-resolve uses same `auto-resolve/<filename> = "true"` keys. Loaded per-worktree via `is_worktree_auto_rebase()` / `load_auto_resolve_files(wt_path)`. `load_auto_rebase_from_worktrees()` scans all worktrees to populate `auto_rebase_enabled: HashSet<String>` on startup.
...
- Codex: `codex exec --json --model <name> [--config 'model_reasoning_effort="ultra"'] -` (prompt via stdin; context injection replaces native resume)
...
- Model override: `⌃m` cycles `selected_model` through a unified pool of model aliases and supported Codex effort entries (Claude + Codex in one cycle). The cycle begins opus → sonnet → haiku, then exposes each current Codex model and its `model:effort` entries before the remaining frontier aliases, wrapping at the end. Bare `gpt-5.6-sol` remains Azureal's default and means Ultra; explicit entries such as `gpt-5.6-sol:xhigh` are parsed into `--model gpt-5.6-sol` plus `--config 'model_reasoning_effort="xhigh"'`. The current Codex catalog supports low/medium/high/xhigh for standard models, max for Sol/Terra/Luna, and ultra for Sol/Terra. Colors are derived by family via `model_color()` and effort suffixes inherit the base model color. Backend is derived dynamically from the selected model via `backend_for_model()` — gpt-* entries use Codex backend, all others use Claude. `AgentProcess` holds both backends and dispatches at spawn time. The selected label is passed through `spawn()` and parsed only by Codex argument construction; Claude receives its normal alias. `detected_model` (separate field) is used only for context window heuristics, not display. The selected model is threaded through `AgentProcessor::spawn()`/`reset()` into `CodexEventParser`, which embeds it in `Init` events so the renderer can derive the correct agent label ("Claude"/"Codex") and color from the model string. Chat bubble headers show agent name left-aligned and model/effort ID right-aligned (subdued style). **Backend availability gating:** At startup, `Config::is_backend_installed()` probes PATH for the `claude`/`codex` executables (via `which` on Unix, `where` on Windows). Results stored as `app.claude_available` / `app.codex_available` (both default `true`). `available_models()` filters `ALL_MODELS` to only include entries whose backend is installed — if neither is found, falls back to the full pool (the app can't function without at least one). `cycle_model()` and `restore_model_from_session()` both use this filtered pool so users never land on an entry they can't run. If the default model's backend is unavailable, the default shifts to the first available entry. Detection runs before `load_session_output()` to ensure the initial model restore respects availability. Implementation: `src/config.rs` (`is_backend_installed()`), `src/app/state/app.rs` (`claude_available`/`codex_available` fields), `src/app/state/app/model.rs` (`available_models()`/`first_available_model()`), `src/tui/run.rs` (detection + initial model override). **Model persistence:** `restore_model_from_session()` is called at the end of every `load_session_output()` — on startup, worktree switch, project switch, and session list selection. It calls `last_session_model()` which scans `display_events` in reverse for `ModelSwitch` tags first (explicit user choice, highest priority), then falls back to `Init` events, mapping model strings back to `ALL_MODELS` aliases via `model_alias_from_init()`. Updates both `selected_model` and `backend`. Falls back to `first_available_model()` for empty/unrecognized sessions, or when the restored entry's backend is not installed. **Auto-spawned processes follow model switcher:** RCR, GFM, DH, run-command generation, issue generation, auto-continue, compaction, and commit-message generation resolve a missing selected model to bare `gpt-5.6-sol` before spawning when Codex is available. If the user has a Codex model/effort selected, auto-spawned processes use that Codex configuration; if Claude, they use Claude.
...
- **Deleting** removes the worktree directory AND deletes the git branch permanently — both local and remote (`Wd` leader sequence). Opens a centered dialog box with red double-border. **Safety warnings:** before showing the dialog, counts uncommitted changes on the worktree path (skipped for archived) using the same gitignore-aware filtering as the git panel (`git diff HEAD --name-only` + `git ls-files --others --exclude-standard` + `git check-ignore --stdin`), and runs `git log main..branch --oneline` from the repo root. If uncommitted changes or unmerged commits are found, yellow `! N uncommitted change(s)` / `! N commit(s) not merged to main` warning lines appear in the dialog between the question and the action keys. Does not block deletion — just warns. **Sibling guard:** if other worktrees share the same branch, the dialog offers to delete all siblings + branch (`y`) or archive the current worktree only (`a`) — git prevents branch deletion while worktrees are checked out. Sole worktrees show a simple y/Esc confirmation. State: `delete_worktree_dialog: Option<DeleteWorktreeDialog>` enum with `Sole { name, warnings }` and `Siblings { branch, sibling_indices, count, warnings }` variants. Also cleans up auto-rebase config and all session state maps (`session_files`, `session_selected_file_idx`, `claude_session_ids`, `branch_slots`, `active_slot`, `running_sessions`, `claude_receivers`, `claude_exit_codes`, `unread_sessions`) for the deleted branch. `delete_branch()` deletes local branch, pushes `--delete` to remote, and prunes the local remote-tracking ref (`origin/<branch>`) so stale refs don't appear in branch dialogs.
...
- **Cross-machine cleanup:** On startup and project switch, `Git::prune_remote_refs()` runs `git remote prune origin` then deletes local `{branch_prefix}/*` branches that are fully merged to main and have no remote counterpart. Prevents worktrees deleted on one machine from appearing as archived on another.
...
- Process killing (`src/backend.rs`, `src/app/state/ui.rs`): `kill_process_tree(pid)` sends SIGTERM to the entire process group (Unix: `libc::killpg(-pgid, SIGTERM)`; Windows: `taskkill /PID /T /F`). Agents are spawned with `process_group(0)` (own process group leader) so all children (cargo test, subagents, etc.) are reached. `kill_process_tree_force(pid)` (Unix only) sends SIGKILL — used during app quit after a 200ms grace period to eliminate any survivors that ignored SIGTERM. **Quit flow:** `cancel_all_claude()` sends SIGTERM to all tracked PIDs and returns them; the event loop sleeps 200ms then calls `kill_process_tree_force()` on each. **Commit gen PID tracking:** Commit message generation processes spawned via `std::process::Command` (no streaming channel) are registered in `commit_gen_pids: Arc<Mutex<Vec<u32>>>` on `App` and included in `cancel_all_claude()` so they are killed on quit. Claude subprocess spawned with `.stdin(Stdio::null())` to prevent console stdin handle sharing on Windows (causes input event competition between TUI and child).
...
- Kitty keyboard protocol (`src/tui/run.rs` entry point): `PushKeyboardEnhancementFlags` (DISAMBIGUATE_ESCAPE_CODES only) gated to `#[cfg(not(target_os = "windows"))]` — conflicts with mouse capture on Windows Terminal. REPORT_EVENT_TYPES deliberately omitted: Kitty sends legacy single-byte sequences (`\t`/`\r`/`\x7F`) for both press AND release of Tab/Enter/Backspace, and crossterm decodes both as `KeyEventKind::Press`, causing double-fire (Kitty #8212). Detection: crossterm DSR query ORed with `term_program_supports_kitty()` env var check (iTerm.app/kitty/ghostty) to catch false negatives. WezTerm deliberately excluded — it accepts the push silently but doesn't honor the protocol on macOS.
- Bracketed paste (`src/tui/run.rs` entry point): `EnableBracketedPaste` sent on startup, `DisableBracketedPaste` on exit (all platforms). Tells the terminal to wrap pasted content in `CSI 200~` / `CSI 201~` escape sequences so crossterm delivers it as `Event::Paste(String)` instead of individual key events. Without this, multi-line paste sends each newline as `Enter`, which submits the prompt. `process_input_event()` in `src/tui/event_loop/process_input.rs` handles `Event::Paste`: prompt mode inserts text at cursor preserving newlines, terminal mode forwards to PTY with bracketed paste wrapping, edit mode inserts character-by-character.
- Paste coalescing (`src/tui/event_loop.rs`): On Windows, crossterm's `ReadConsoleInputW`-based reader delivers pasted text as individual `KEY_EVENT` records — `Event::Paste` is never generated despite `EnableBracketedPaste`. Two complementary mechanisms handle this: **(1) Batch coalescing:** The event loop's drain collects all events into a batch and passes them through `coalesce_paste_events()` which detects paste patterns: ≥3 char/Enter Press events with characters appearing after an Enter (impossible via human typing). Detected pastes are coalesced into a single `Event::Paste(text)`. Drain extension: when a batch has ≥3 key presses OR ends with Enter, the loop waits up to 5ms for more events. **(2) Deferred Enter (`paste_deferred_enter`):** The primary Windows paste fix. When Enter is pressed in prompt mode on Windows, submission is deferred for 30ms. If characters arrive within that window (from a paste), the Enter is converted to a `\n` character in the input instead of triggering submit. The event loop checks `paste_deferred_enter` every tick; after 30ms with no new chars, it re-injects Enter with `paste_deferred_enter` still set so `handle_input_mode` skips the defer and proceeds to submit. `paste_deferred_enter` is included in `is_busy` so the event loop polls at 16ms during the defer window. In `handle_input_mode`: Char after defer → insert `\n` + process char; Enter after defer → insert `\n` + re-defer; non-char key → cancel defer. **Paste guard safety net:** `paste_guard_until` (20ms `Instant` on App, set on prompt submit) suppresses single-letter globals via `KeyContext.paste_guard` in `lookup_action()` skip conditions. This prevents paste remnants like `Shift+P` from triggering `OpenProjects`.
...
### REPORT_EVENT_TYPES causes double-fire for Tab/Enter/Backspace in Kitty

With `DISAMBIGUATE_ESCAPE_CODES | REPORT_EVENT_TYPES` (Kitty keyboard mode 3), Kitty sends both press and release events. But Tab (`\t`), Enter (`\r`), and Backspace (`\x7F`) use **legacy single-byte sequences** for both events — the same byte for press and release. Crossterm's parser cannot distinguish them and decodes both as `KeyEventKind::Press`, so the input thread's Release filter never fires. Result: every Tab/Enter/Backspace fires twice.

```rust
// ❌ WRONG — causes double-fire for Tab/Enter/Backspace in Kitty
PushKeyboardEnhancementFlags(
    KeyboardEnhancementFlags::DISAMBIGUATE_ESCAPE_CODES
        | KeyboardEnhancementFlags::REPORT_EVENT_TYPES
)

// ✅ CORRECT — DISAMBIGUATE alone gives Shift+Enter distinction without double-fire
PushKeyboardEnhancementFlags(
    KeyboardEnhancementFlags::DISAMBIGUATE_ESCAPE_CODES
)
```

**Affected:** Kitty terminal (and potentially other terminals implementing the Kitty keyboard protocol). Kitty issue #8212 confirms this is spec-undefined behavior for flag combo mode 3. Fixed by removing `REPORT_EVENT_TYPES` — DISAMBIGUATE alone provides Shift+Enter/Ctrl+M distinction.
...
**Startup sequence** (`src/tui/run.rs::run`): `load_global_azufig()` → (if `show_startup_screen`) `draw_splash()` → `App::new()` → `app.load()` → `app.load_session_output()` → `event_loop::run_app()`. The global azufig is loaded first to check `show_startup_screen` before any drawing. The splash screen renders immediately after terminal init (before any App state exists) so the user sees the AZUREAL logo with a dim spring azure butterfly outline (the app mascot) in the background while git discovery and session parsing run. Two-layer rendering: butterfly background using outlined wings (box-drawing chars + `░` fill, `║` body column, `╱╲` antennae) at `Color::Rgb(15, 45, 80)`, then logo + half-block acronym + "Loading project..." on top overwriting butterfly cells where they overlap. Both layers share the same vertical center point so butterfly wings extend above and below the text. Displayed for minimum 3 seconds (skipped entirely when `show_startup_screen` is false). The first `ui()` draw in `run_app()` replaces the splash with the full layout.
...
Multi-line input is supported via Shift+Enter. The Kitty keyboard protocol is enabled on startup via `PushKeyboardEnhancementFlags` (DISAMBIGUATE_ESCAPE_CODES only). REPORT_EVENT_TYPES is omitted because Kitty sends legacy single-byte sequences for Tab/Enter/Backspace on both press and release, causing crossterm to decode both as Press → double-fire (Kitty #8212). REPORT_ALL_KEYS_AS_ESCAPE_CODES is omitted because it causes Shift+letter to arrive as `(SHIFT, Char('1'))` instead of `(NONE, Char('!'))`, breaking secondary character input. With DISAMBIGUATE alone, Shift+Enter sends `CSI 13;2u` → `(SHIFT, Enter)`, which is sufficient. An `(ALT, Enter)` arm is kept as a safety net for Kitty-macOS edge cases. Both Press and Repeat events are processed (Repeat fires when a key is held down, enabling fast cursor movement with held arrow keys). The input field dynamically grows in height (up to 3/4 of terminal height) with proper cursor positioning for newlines and character-level wrapping. When content exceeds the visible area, the view scrolls to keep the cursor visible.
...
Implementation: `prompt_mode: bool`, `paste_guard_until: Instant` in `App` struct, border color logic in `draw_input()` in `src/tui/draw_input.rs`.
...
- **`bindings.rs`** — ~22 static arrays per context: `GLOBAL` (19 entries — core globals: `⌃q`, `⌃d`, cancel, copy, `⌃m`, `?`, `p`, `T`, `G` OpenGitActions, `H` OpenHealth, `I` OpenIssues, `M` BrowseMain, `P` OpenProjects, `]`/`[` worktree tabs, `r` RunCommand, `R` AddRunCommand, `Tab`/`⇧Tab`), `WORKTREES` (4 entries — leader sequence `W <key>` targets: `n` AddWorktree, `r` RenameWorktree, `a` ToggleArchive, `d` DeleteWorktree), `FILE_TREE` (17 entries), `VIEWER`, `EDIT_MODE`, `SESSION`, `INPUT`, `TERMINAL`, `HEALTH_SHARED` (9 entries), `HEALTH_GOD_FILES` (4 entries), `HEALTH_DOCS`, `GIT_ACTIONS` (27 entries — context-aware, includes BrowseMain), `ISSUES_BROWSE` (10 entries), `PROJECTS_BROWSE`, `PICKER`, `BRANCH_DIALOG`, `AZUREAL_SHARED`, `AZUREAL_DEBUG`, `AZUREAL_ISSUES`, `AZUREAL_PRS`. Plus `ALT_*` static arrays for dual-key alternatives
- **`lookup.rs`** — `KeyContext` (captures guard state from App: focus, prompt_mode, edit_mode, terminal_mode, filter_active, help_open, stt_recording, paste_guard; built via `KeyContext::from_app(app)`), `lookup_action()` with guard logic inside (skip conditions prevent globals from firing during text input, edit mode, filter, or paste guard — terminal type mode (`prompt_mode=true`) blocks single-letter globals; terminal command mode allows all globals; `paste_guard` (true for 20ms after prompt submit) blocks single-letter globals to prevent paste remnants from triggering actions like OpenProjects on Windows; `EnterPromptMode` (`p`) has its own narrower skip — only blocked in edit mode, paste_guard, or when prompt is already focused (so `p` refocuses when tabbed away); no guard duplication in event_loop.rs; when `stt_recording` is true, ToggleStt resolves from any focus/mode; `Focus::Worktrees` maps to `&WORKTREES` so `n`/`r`/`a`/`d` resolve directly when the worktrees panel is focused — leader sequence still works from any focus), `lookup_leader_action(mods, code)` resolves second key of `W` leader sequence against the WORKTREES binding array, plus 7 per-modal lookup functions: `lookup_health_action(tab, mods, code)`, `lookup_git_actions_action(focused_pane, is_on_main, mods, code)`, `lookup_azureal_action(tab, mods, code)`, `lookup_projects_action(mods, code)`, `lookup_picker_action(mods, code)`, `lookup_branch_dialog_action(mods, code)`
...
**Unborn / strictly-behind fast-forward paths:** `git rebase` fails when the worktree's branch has no commits (unborn HEAD — e.g., changes were made to main in another worktree, leaving this worktree's branch with nothing to replay). It also runs an unnecessary rebase when the branch is strictly behind main (HEAD is already an ancestor of main). `exec_rebase_inner()` probes both conditions before falling through to the normal rebase path. The `is_unborn_head()` helper (`pub(crate)` in `operations.rs`, re-exported from the module root) detects unborn branches via `git rev-parse --verify HEAD` returning non-zero. On unborn HEAD, it runs `git reset --mixed <main>` to move the branch ref to main's tip and reset the index to match — `--mixed` clears any stale staging (e.g. files added with `git add .` while the branch was unborn would otherwise stay staged at old-main content and appear as reverts of main's newer state) while leaving the working tree alone so user edits are preserved. Stash isn't usable here — `git stash` requires HEAD to exist and fails on unborn branches. When HEAD exists and `git merge-base --is-ancestor HEAD <main>` succeeds (strictly behind), it stashes dirty working tree with `--include-untracked`, runs `git merge --ff-only <main>`, then pops the stash on all exit paths. Both paths return `RebaseOutcome::Rebased` on success. Dirty-worktree skip guards in both manual rebase (`exec_rebase()` changed-files check) and auto-rebase (`check_auto_rebase()` `git status --porcelain` check) bypass the block when `is_unborn_head()` returns true, since `reset --mixed` doesn't fail on a dirty working tree (only `--hard` would).
...
- `⇧Enter` / `⌃j` / `⌥Enter` — Insert a newline in the Command/Prompt field. The field wraps and expands vertically up to the available dialog height, then scrolls to keep the cursor visible.
- `⌘/Ctrl+A/C/X/V` — Select all, copy, cut, and paste in the active field. Bracketed paste is routed into the active dialog field instead of the main prompt.
- `Esc` — Cancel
...
3. **Worktree** - create git worktree with Claude session
   - Name: becomes `{branch_prefix}/{name}` branch (prefix derived from git remote origin repo name, falling back to folder name)
...
Cross-platform notification sent when any agent instance finishes its response. Fires for every session exit (not just the currently viewed one), so the user sees alerts even when working in another app. **Project-aware:** Background project sessions use the snapshot's project name (not the currently active project), so `AZUREAL @ iDiOSONiX` shows correctly even when the user is viewing the AZUREAL project.
...
```
azureal/
├── .azureal/                # Project-level azureal data (ignored locally by Azureal)
...
| models | `src/models.rs` | 68 | `strip_branch_prefix` (16 -- any-prefix/no-slash/empty/nested-slash/different-prefix/unicode/emoji/double-prefix/case-insensitive-strips/dots/dashes/partial-match/whitespace/slash-only), `branch_prefix_for_path` (6 -- repo-name-lowercased/spaces-to-dashes/mixed-case/root-fallback/dashes-preserved/numeric), `WorktreeStatus` (12 -- as_str/symbol/color all variants, PartialEq exhaustive 6x6, Eq, Copy, Clone, Debug, serde roundtrip, lowercase/non-empty validation), `Worktree` (17 -- name edge cases, construction variations, full 8-combination status truth table, clone, debug, serde roundtrip with/without None fields), `DiffInfo` (4 -- construction, empty, clone, serde roundtrip), `OutputType` (7 -- all variants, PartialEq exhaustive, Copy, Clone, Debug, serde roundtrip), `RebaseResult` (5 -- Aborted, Failed with/without message, clone, debug) |
...
| state/app | `src/app/state/app.rs` | 122 | `App::new()` field defaults (60+ -- project/worktrees/focus/view_mode/session/terminal/viewer/render/parse_stats/tokens/todos/stt/file_tree/health/git/browsing/session_list/find/filter/model/backend), `invalidate_render_cache` (2 -- sets dirty, idempotent), `invalidate_file_tree` (1), `set_status`/`clear_status` (6 -- stores/overwrites/String/format/clears/noop), `current_project`/`current_worktree` (5 -- none defaults, selected index, browsing_main returns main, out of bounds), `is_session_running` (3 -- no slots, running slot, stopped slot), `is_current_session_running` (2 -- no worktree, true), `is_active_slot_running` (3 -- no worktree, running, stopped), `branch_for_slot` (3 -- found, not found, empty), `is_claude_session_running` (3 -- true, not running, no match), `display_model_name` (2 -- default gpt-5.6-sol when None, custom value), `backend_for_model` (3 -- Claude models, Codex models, unknown defaults Claude), `cycle_model` (5 -- opus→sonnet, haiku→gpt-5.6-sol crosses backend, effort variants, last codex wraps to opus, full 46-entry cycle with backend checks, unknown→gpt-5.6-sol:low), `ALL_MODELS` (3 -- has 46 entries, contains default, Claude then Codex ordering), `update_token_badge` (6 -- no tokens, low/medium/high usage, default 200k window, drop below threshold), `TodoItem`/`TodoStatus` (6 -- equality, construction, clone, debug), `DeferredAction` (7 -- all variant construction), `cancel_all_claude` (1 -- clears all state), `git_action_in_progress` (4 -- default false, deferred commit/commit+push, non-git deferred), `get_cpu_time_micros` (1 -- returns nonzero) |
...
| tui/input_git_actions/operations | `src/tui/input_git_actions/operations.rs` | 58 | `parse_conflict_files` (5 -- mixed/auto-merged-only/whitespace-trimming), auto-resolve boundary checks, overlay field construction (GitConflictOverlay/GitCommitOverlay/GitChangedFile/PostMergeDialog), commit message markdown fence stripping, generator label shows selected model, Sol Ultra model arguments, push note formatting |
...
| backend | `src/backend.rs` | 16 | `Backend` default (Codex), Copy/Clone/Debug/PartialEq, display format, from_str_loose (5 -- claude/codex/openai/unknown/empty/case-insensitive), serde roundtrip (3 -- codex/claude/unknown fails), equality (1), clone (1), debug (1), `AgentProcess::new` (1 -- constructs both backends), spawn empty prompt (3 -- claude/codex/no-model all fail) |
...
| codex | `src/codex.rs` | 26 | Process configuration and empty-prompt validation, stdin/resume argument construction, permission flag mapping (Ignore/Approve/Ask), model/effort argument construction including Sol Ultra, oversized context trimming, event-channel ordering, and thread/session id extraction |
...
| `O` | Options overlay (toggle visibility of `.git`, `.claude`, `.azureal`, `target`, `node_modules` dirs) |
...
| `J/K` | Page scroll (viewport minus 2 overlap) |
| `⌥↑/⌥↓` (`Alt+↑/↓`) | Jump to top/bottom |
| `n` | New session (clear state, enter prompt mode — also works from session list overlay) |
...
| `n/N` | Next/prev search match only while confirmed `/` search matches are active |
