...
- **Project-local** `.azureal/azufig.toml` — filetree options (hidden entry names), health scan scope (directory paths), project-local run commands, project-local preset prompts. Always at the **main worktree root** (resolved via `git rev-parse --git-common-dir` parent), shared by all worktrees — no per-worktree copies. **Gitignored by default** — `.azureal/` is added to `.gitignore` by `ensure_worktrees_gitignored()` alongside `worktrees/` to prevent the session store and runtime files from causing rebase conflicts.
