...
- **Hide .git toggle in Git panel** — Top-right corner of the Git panel shows `h:hide .git ✓` (green) or `h:hide .git ✗` (red). Press `h` to toggle `.git` directory visibility in the file tree. Hidden by default. Rebuilds file tree immediately on toggle.
