...
- **God File filter mode** — Press `f` in the God File panel to open the FileTree in filter mode, showing which directories are included in the scan scope. Scoped directories highlighted bright green with green icons; files inside scope dimmed green; everything else dimmed gray. Green double-line border with "God File Scope (N dirs)" title and "Enter:toggle  Esc:rescan" bottom hint. Press Enter on any directory to add/remove it from the scan scope. Esc rescans with the modified scope and returns to the god file panel with updated results. File modification actions (add, delete, rename, copy, move) disabled during filter mode.
