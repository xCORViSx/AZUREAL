...
### Added
- **MCR (Merge Conflict Resolution) interactive mode** — When squash merge encounters conflicts and Claude is spawned to resolve them, the convo pane enters MCR mode: green borders and title `[MCR] <worktree>` visually indicate active conflict resolution. The session is persistent and interactive — user can send follow-up prompts routed to the repo root with `--resume`. When Claude exits, a centered green-bordered approval dialog asks "Accept conflict resolution?" with `[y] Accept` / `[n] Review more`. Pressing `n` dismisses (status shows `⌃a to accept`); pressing `y` deletes the temporary session file, clears MCR state, and restores normal borders. `⌃a` re-shows the dialog after dismissing. Title locked during MCR. State tracked by `McrSession` struct on `App`. 9 files modified.
- **Merge conflict resolution dialog** — When squash merge encounters conflicts, a red-bordered `GitConflictOverlay` opens on top of the Git panel listing conflicted files (red) and auto-merged files (green). Two actions: `[y] Resolve with Claude` spawns a streaming Claude session (named `[MCR] <branch>`) in MCR mode; `[n] Abort merge` runs `git merge --abort`. Closing the git panel while the overlay is open auto-aborts the merge. `squash_merge_into_main()` returns `SquashMergeResult` enum. `handle_git_actions_input()` takes `&ClaudeProcess`.

### Fixed
- **Squash merge fails when main has dirty working tree** — Uncommitted files on main (e.g. `.DS_Store`) caused `git merge --squash` to abort with "your local changes would be overwritten". `squash_merge_into_main()` now stashes any dirty state before merging (`git stash --include-untracked`) and pops after commit. Stash is also popped on merge abort and non-conflict failures. On conflict, stash is left in place (merge state is dirty); `merge_abort()` pops it when cleaning up.
- **Squash merge with uncommitted changes loses work** — Squash merging while the feature branch has uncommitted changes would exclude those changes from the merge. `exec_squash_merge()` now checks `changed_files` and shows "Commit your changes first (c) before squash merging" error if any exist.
- **Merge resolution convo disappears after Claude exits** — Streaming output from conflict-resolution Claude sessions vanished when the process exited. Root cause: `handle_claude_exited()` forced a full session-file re-parse on exit, which loaded the OLD session file (from the feature branch's directory) and replaced `display_events`. The merge session's file lives under main's directory (where Claude was spawned), so the re-parse clobbered the streaming output. Fix: skip the re-parse when the exiting slot's session file doesn't exist in the current worktree's session directory.

### Changed
- **Health panel title shows worktree name** — Panel title changed from static `" Worktree Health "` to `" Health: <worktree> "`, matching the Git panel's `" Git: <worktree> "` naming pattern. `HealthPanel` struct now stores `worktree_name` populated from the selected worktree at open time.
...
### Added
- **Read-only main branch isolation** — Main/master branch is no longer in `app.worktrees` — stored separately in `app.main_worktree: Option<Worktree>` and hidden from the sidebar worktree list entirely. Press `m` from the Worktrees context to enter read-only browse mode: file tree opens showing main's files with yellow "(read-only)" border suffix, sidebar title shows "★ main (read-only)" in yellow. All file mutations blocked (add/delete/rename/copy/move), edit mode blocked, prompt mode blocked, session start blocked. `Esc` exits browse mode and restores previous worktree selection. `current_worktree()` returns `main_worktree` when `browsing_main` is true (transparent to all downstream code). Star icon removed from sidebar (main no longer in list). `BrowseMain` action added to `keybindings.rs`, `WORKTREES` array now 15 entries. `enter_main_browse()` / `exit_main_browse()` in `ui.rs` manage state; `switch_project()` clears browse state. Health panel (GFM/DH sessions) now spawns on current worktree via `current_worktree_info()` (replaced `find_main_worktree()` + `switch_to_main_worktree()`). Run command prompt spawns on current worktree. 14 files modified across app state, TUI rendering, input handling, and keybindings.
- **Context-aware git panel actions** — Git panel actions now change based on whether the current worktree is on the main/master branch or a feature branch. On main: pull (`l`), commit (`c`), push (`P`). On feature branches: squash-merge (`m`), commit (`c`), push (`P`). `is_on_main: bool` field added to `GitActionsPanel`, set by comparing `worktree_name == main_branch` when opening the panel. `GitPull` action added to keybindings with `l` binding. `lookup_git_actions_action()` and `git_actions_labels()` now take `is_on_main` parameter. Guards enforce mutual exclusivity: `GitSquashMerge` blocked on main, `GitPull` blocked on feature branches. `GIT_ACTIONS` array increased from 12 to 13 entries. Confirm index 0 dispatches pull on main or squash-merge on feature.
- **Git push as separate panel action** — `Shift+P` in the Git panel pushes the current branch to remote. Squash merge (`m`) no longer auto-pushes after committing; push is now an explicit user action. Git panel increased from 2 to 3 actions (squash-merge, commit, push). `GIT_ACTIONS` array increased from 11 to 12 entries. `ACTION_COUNT` updated to 3 with confirm-index mapping: 0=squash-merge, 1=commit, 2=push.
...
### Changed
- **Main branch isolated from sidebar** — Main/master worktree no longer appears in the sidebar worktree list. Star icon (`★`) removed from sidebar; instead, browsing main shows a yellow "★ main (read-only)" sidebar title. Main stored in `app.main_worktree` (separate from `worktrees` vec) during `load_worktrees()`.
- **Health panel spawns on current worktree** — GFM and DH sessions now spawn on the currently selected worktree instead of always targeting main. `find_main_worktree()` and `switch_to_main_worktree()` removed; replaced with `current_worktree_info()` which returns the current worktree's branch name and path. Changes from health sessions merge back to main via squash-merge.
- **Run command prompt spawns on current worktree** — `spawn_run_command_prompt()` in `input_dialogs.rs` now uses `app.current_worktree()` instead of finding the main worktree.
...
- **Replaced rebase/merge with squash merge in Git panel** — All merge/rebase actions replaced with squash merge + explicit push. Git panel reduced from 7 actions to 3 (`m` squash-merge, `c` commit, `Shift+P` push). `m` key performs a squash merge cycle: (1) `git pull --ff-only` on main (non-fatal if offline, fatal if diverged), (2) `git merge --squash <branch>`, (3) `git commit -m "feat: merge <branch> into main"`. Push is a separate user action (`Shift+P`). `GitFetch`, `GitPull` action variants removed from keybindings; `GitPush` re-added as a standalone action. `GIT_ACTIONS` array reduced from 14 to 12 entries. Reports conflicts with count and auto-merged file count; returns "Already up to date" when nothing to merge. `Git::squash_merge_into_main()` in `src/git/core.rs` owns the pull→merge→commit cycle (no push). `RebaseFromMain` removed from `WorktreeAction` enum. Auto-rebase on Claude exit removed entirely.

### Removed
- **`find_main_worktree()` and `switch_to_main_worktree()`** — Replaced by `current_worktree_info()` in `health.rs`. Health sessions no longer need to locate or switch to main specifically.
- **Star icon in sidebar worktree list** — Main branch is no longer in the worktree list at all. The star visual is now part of the browse-main sidebar title only.
- **Auto-rebase on Claude exit** — `Shift+A` toggle, scope picker, per-worktree `auto-rebase` azufig config, and `auto_rebase_on_exit()` in `claude_events.rs` all removed. Claude sessions no longer auto-rebase after exiting.
- **Rebase from main action** — `r` key in Git panel no longer rebases. `Git::rebase_onto_main()` has no entry points. `src/git/rebase.rs` and `src/tui/input_rebase.rs` preserved on disk as dead code but `input_rebase` removed from `tui.rs` module list.
- **Merge to main action** — Replaced by squash merge. `Git::merge_into_main()` replaced by `Git::squash_merge_into_main()`.
