...
mod tests {
    ...
    fn global_ctrl_d_dumps_debug() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::CONTROL, KeyCode::Char('d')), Some(Action::DumpDebug));
    }
    ...
    fn global_ctrl_m_cycles_model() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::CONTROL, KeyCode::Char('m')), Some(Action::CycleModel));
    }

    #[test]
    fn global_question_toggles_help() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('?')), Some(Action::ToggleHelp));
    }
    ...
    fn global_shift_t_toggles_terminal() {
        ...
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('T')), Some(Action::ToggleTerminal));
    }

    #[test]
    fn global_shift_g_opens_git() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::SHIFT, KeyCode::Char('G')), Some(Action::OpenGitActions));
    }

    #[test]
    fn global_shift_h_opens_health() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::SHIFT, KeyCode::Char('H')), Some(Action::OpenHealth));
    }

    #[test]
    fn global_shift_m_browses_main() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::SHIFT, KeyCode::Char('M')), Some(Action::BrowseMain));
    }

    #[test]
    fn global_shift_p_opens_projects() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::SHIFT, KeyCode::Char('P')), Some(Action::OpenProjects));
    }

    #[test]
    fn global_tab_cycles_focus_forward() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Tab), Some(Action::CycleFocusForward));
    }

    #[test]
    fn global_backtab_cycles_focus_backward() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::BackTab), Some(Action::CycleFocusBackward));
    }

    #[test]
    fn global_bracket_right_next_worktree() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char(']')), Some(Action::WorktreeTabNext));
    }

    #[test]
    fn global_bracket_left_prev_worktree() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('[')), Some(Action::WorktreeTabPrev));
    }
    ...
    fn worktrees_cmd_d_deletes() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::SUPER, KeyCode::Char('d')), Some(Action::DeleteWorktree));
    }
    ...
    fn filetree_k_navigates_up() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('k')), Some(Action::NavUp));
    }

    #[test]
    fn filetree_enter_opens_file() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Enter), Some(Action::OpenFile));
    }

    #[test]
    fn filetree_space_toggles_dir() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char(' ')), Some(Action::ToggleDir));
    }

    #[test]
    fn filetree_a_adds_file() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::AddFile));
    }

    #[test]
    fn filetree_d_deletes_file() {
        let ctx = cmd_ctx(Focus::FileTree);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('d')), Some(Action::DeleteFile));
    }
    ...
    fn viewer_e_enters_edit() {
        let ctx = cmd_ctx(Focus::Viewer);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('e')), Some(Action::EnterEditMode));
    }
    ...
    fn session_a_new_session() {
        ...
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::NewSession));
    }

    #[test]
    fn session_s_session_list() {
        let ctx = cmd_ctx(Focus::Session);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('s')), Some(Action::ToggleSessionList));
    }

    #[test]
    fn session_slash_search() {
        let ctx = cmd_ctx(Focus::Session);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Char('/')), Some(Action::SearchSession));
    }

    #[test]
    fn session_down_next_bubble() {
        let ctx = cmd_ctx(Focus::Session);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Down), Some(Action::JumpNextBubble));
    }

    #[test]
    fn session_up_prev_bubble() {
        let ctx = cmd_ctx(Focus::Session);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::Up), Some(Action::JumpPrevBubble));
    }
    ...
    fn unknown_key_returns_none() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::NONE, KeyCode::F(12)), None);
    }

    #[test]
    fn random_ctrl_combo_returns_none() {
        let ctx = cmd_ctx(Focus::Worktrees);
        assert_eq!(lookup_action(&ctx, KeyModifiers::CONTROL, KeyCode::Char('x')), None);
    }
    ...
    fn health_tab_switches_tab() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Tab), Some(Action::HealthSwitchTab));
    }

    #[test]
    fn health_esc_closes() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Esc), Some(Action::Escape));
    }

    #[test]
    fn health_j_navigates_down() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char('j')), Some(Action::NavDown));
    }

    #[test]
    fn health_k_navigates_up() {
        assert_eq!(lookup_health_action(HealthTab::Documentation, KeyModifiers::NONE, KeyCode::Char('k')), Some(Action::NavUp));
    }

    #[test]
    fn health_god_files_space_toggles_check() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char(' ')), Some(Action::HealthToggleCheck));
    }

    #[test]
    fn health_god_files_a_toggles_all() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::HealthToggleAll));
    }

    #[test]
    fn health_god_files_v_views_checked() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char('v')), Some(Action::HealthViewChecked));
    }

    #[test]
    fn health_god_files_enter_modularizes() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Enter), Some(Action::HealthModularize));
    }

    #[test]
    fn health_god_files_m_modularizes() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char('m')), Some(Action::HealthModularize));
    }

    #[test]
    fn health_docs_space_toggles_check() {
        assert_eq!(lookup_health_action(HealthTab::Documentation, KeyModifiers::NONE, KeyCode::Char(' ')), Some(Action::HealthDocToggleCheck));
    }

    #[test]
    fn health_docs_a_toggles_non_100() {
        assert_eq!(lookup_health_action(HealthTab::Documentation, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::HealthDocToggleNon100));
    }

    #[test]
    fn health_docs_v_views_checked() {
        assert_eq!(lookup_health_action(HealthTab::Documentation, KeyModifiers::NONE, KeyCode::Char('v')), Some(Action::HealthViewChecked));
    }

    #[test]
    fn health_docs_enter_spawns() {
        assert_eq!(lookup_health_action(HealthTab::Documentation, KeyModifiers::NONE, KeyCode::Enter), Some(Action::HealthDocSpawn));
    }

    #[test]
    fn health_unknown_key_returns_none() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::F(5)), None);
    }

    #[test]
    fn health_shared_scope_mode() {
        assert_eq!(lookup_health_action(HealthTab::GodFiles, KeyModifiers::NONE, KeyCode::Char('s')), Some(Action::HealthScopeMode));
    }
    ...
    #[test]
    fn git_esc_closes() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Esc), Some(Action::Escape));
    }

    #[test]
    fn git_tab_toggles_focus() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Tab), Some(Action::GitToggleFocus));
    }

    #[test]
    fn git_squash_merge_on_feature_actions_focused() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('m')), Some(Action::GitSquashMerge));
    }

    #[test]
    fn git_squash_merge_skipped_on_main() {
        assert_ne!(lookup_git_actions_action(0, true, KeyModifiers::NONE, KeyCode::Char('m')), Some(Action::GitSquashMerge));
    }

    #[test]
    fn git_squash_merge_skipped_when_not_actions_focused() {
        assert_ne!(lookup_git_actions_action(1, false, KeyModifiers::NONE, KeyCode::Char('m')), Some(Action::GitSquashMerge));
    }
    ...
    fn git_pull_on_main_actions_focused() {
        assert_eq!(lookup_git_actions_action(0, true, KeyModifiers::NONE, KeyCode::Char('l')), Some(Action::GitPull));
    }

    #[test]
    fn git_pull_skipped_on_feature() {
        assert_ne!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('l')), Some(Action::GitPull));
    }

    #[test]
    fn git_pull_skipped_when_not_actions_focused() {
        assert_ne!(lookup_git_actions_action(1, true, KeyModifiers::NONE, KeyCode::Char('l')), Some(Action::GitPull));
    }

    #[test]
    fn git_commit_actions_focused() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('c')), Some(Action::GitCommit));
    }

    #[test]
    fn git_commit_skipped_not_actions_focused() {
        assert_ne!(lookup_git_actions_action(1, false, KeyModifiers::NONE, KeyCode::Char('c')), Some(Action::GitCommit));
    }

    #[test]
    fn git_push_actions_focused() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::SHIFT, KeyCode::Char('P')), Some(Action::GitPush));
    }

    #[test]
    fn git_push_skipped_not_actions_focused() {
        assert_ne!(lookup_git_actions_action(1, false, KeyModifiers::SHIFT, KeyCode::Char('P')), Some(Action::GitPush));
    }
    ...
    fn git_view_diff_d_from_file_list_resolves_to_confirm() {
        ...
        assert_eq!(lookup_git_actions_action(1, false, KeyModifiers::NONE, KeyCode::Char('d')), Some(Action::Confirm));
    }
    ...
    fn git_auto_rebase_on_feature() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::GitAutoRebase));
    }

    #[test]
    fn git_auto_rebase_skipped_on_main() {
        assert_ne!(lookup_git_actions_action(0, true, KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::GitAutoRebase));
    }

    #[test]
    fn git_auto_resolve_actions_focused() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('s')), Some(Action::GitAutoResolveSettings));
    }

    #[test]
    fn git_auto_resolve_skipped_not_actions_focused() {
        assert_ne!(lookup_git_actions_action(1, false, KeyModifiers::NONE, KeyCode::Char('s')), Some(Action::GitAutoResolveSettings));
    }

    #[test]
    fn git_unknown_key_returns_none() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::F(3)), None);
    }

    #[test]
    fn git_nav_down() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('j')), Some(Action::NavDown));
    }

    #[test]
    fn git_nav_up() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('k')), Some(Action::NavUp));
    }

    #[test]
    fn git_prev_worktree() {
        assert_eq!(lookup_git_actions_action(0, false, KeyModifiers::NONE, KeyCode::Char('[')), Some(Action::GitPrevWorktree));
    }
    ...
    fn projects_enter_confirms() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Enter), Some(Action::Confirm));
    }

    #[test]
    fn projects_a_adds() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::ProjectsAdd));
    }

    #[test]
    fn projects_d_deletes() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Char('d')), Some(Action::ProjectsDelete));
    }

    #[test]
    fn projects_n_renames() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Char('n')), Some(Action::ProjectsRename));
    }

    #[test]
    fn projects_i_inits() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Char('i')), Some(Action::ProjectsInit));
    }

    #[test]
    fn projects_esc_closes() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Esc), Some(Action::Escape));
    }

    #[test]
    fn projects_ctrl_q_quits() {
        assert_eq!(lookup_projects_action(KeyModifiers::CONTROL, KeyCode::Char('q')), Some(Action::Quit));
    }

    #[test]
    fn projects_j_navigates_down() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::Char('j')), Some(Action::NavDown));
    }

    #[test]
    fn projects_unknown_returns_none() {
        assert_eq!(lookup_projects_action(KeyModifiers::NONE, KeyCode::F(9)), None);
    }
    ...
    fn picker_enter_confirms() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Enter), Some(Action::Confirm));
    }

    #[test]
    fn picker_e_edits() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Char('e')), Some(Action::EditSelected));
    }

    #[test]
    fn picker_d_deletes() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Char('d')), Some(Action::DeleteSelected));
    }

    #[test]
    fn picker_a_adds() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Char('a')), Some(Action::ProjectsAdd));
    }

    #[test]
    fn picker_esc_closes() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Esc), Some(Action::Escape));
    }

    #[test]
    fn picker_j_navigates_down() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Char('j')), Some(Action::NavDown));
    }

    #[test]
    fn picker_k_navigates_up() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::Char('k')), Some(Action::NavUp));
    }

    #[test]
    fn picker_unknown_returns_none() {
        assert_eq!(lookup_picker_action(KeyModifiers::NONE, KeyCode::F(7)), None);
    }
    ...
    fn branch_enter_selects() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Enter), Some(Action::Confirm));
    }

    #[test]
    fn branch_esc_closes() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Esc), Some(Action::Escape));
    }

    #[test]
    fn branch_j_navigates_down() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Char('j')), Some(Action::NavDown));
    }

    #[test]
    fn branch_k_navigates_up() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Char('k')), Some(Action::NavUp));
    }

    #[test]
    fn branch_down_arrow_navigates() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Down), Some(Action::NavDown));
    }

    #[test]
    fn branch_up_arrow_navigates() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Up), Some(Action::NavUp));
    }

    #[test]
    fn branch_unknown_returns_none() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::NONE, KeyCode::Char('x')), None);
    }

    #[test]
    fn branch_ctrl_combo_returns_none() {
        assert_eq!(lookup_branch_dialog_action(KeyModifiers::CONTROL, KeyCode::Char('a')), None);
    }
}
