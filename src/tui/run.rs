...
fn draw_splash(f: &mut Frame) {
    ...
    // Celastrina ladon: smooth rounded wings, no tails, no points.
    // Four distinct wings (upper pair larger, lower pair smaller+rounder),
    // thin body down the center, antennae curving outward at top.
    // Uses ░ fill with clear wing separation so it reads as a butterfly.
