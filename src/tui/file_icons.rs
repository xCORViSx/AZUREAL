    ...
    if nerd {
        ...
        match filename_lower.as_str() {
            ...
            ".gitignore" | ".gitmodules" | ".gitattributes" | ".gitkeep" => {
                return ("\u{e65d} ", Color::Rgb(240, 80, 50))
            } // git orange
...
fn nerd_icon_by_ext(path: &Path) -> (&'static str, Color) {
    ...
    match ext.to_ascii_lowercase().as_str() {
        ...
        "py" | "pyi" | "pyw" => ("\u{e606} ", Color::Rgb(53, 114, 165)), // python blue
        "js" | "mjs" | "cjs" => ("\u{e60c} ", Color::Rgb(247, 223, 30)), // js yellow
        ...
        "cpp" | "cc" | "cxx" | "hpp" | "hxx" => ("\u{e61d} ", Color::Rgb(85, 141, 200)), // c++ blue
        ...
        "scala" | "sc" => ("\u{e68e} ", Color::Rgb(220, 50, 47)), // scala red
        "r" => ("\u{e68a} ", Color::Rgb(39, 109, 195)),   // r blue
        "pl" | "pm" => ("\u{e67e} ", Color::Rgb(57, 69, 124)), // perl blue
        "sh" | "bash" | "zsh" | "fish" => ("\u{e691} ", Color::Green), // shell green
        "ps1" | "psm1" | "psd1" => ("\u{e683} ", Color::Rgb(1, 36, 86)), // powershell blue

        // ── Web / markup ──
        "html" | "htm" => ("\u{e60e} ", Color::Rgb(227, 76, 38)), // html orange
        "css" => ("\u{e614} ", Color::Rgb(86, 61, 124)),          // css purple
        "scss" | "sass" => ("\u{e603} ", Color::Rgb(207, 100, 154)), // sass pink
        ...
        // ── Data / config ──
        "json" | "jsonc" | "json5" => ("\u{e60b} ", Color::Yellow),
        "yaml" | "yml" => ("\u{e6a8} ", Color::Rgb(203, 56, 55)), // yaml red
        "toml" => ("\u{e6b2} ", Color::DarkGray),
        "xml" | "xsl" | "xslt" => ("\u{e619} ", Color::Rgb(224, 148, 64)), // xml orange
        "csv" => ("\u{e64a} ", Color::Green),
        "sql" => ("\u{e64d} ", Color::Rgb(85, 141, 200)), // sql blue
        "graphql" | "gql" => ("\u{e662} ", Color::Rgb(225, 0, 152)), // graphql pink
        "proto" => ("\u{e615} ", Color::LightBlue),
        "ini" | "cfg" | "conf" => ("\u{e615} ", Color::DarkGray),

        // ── Documentation ──
        "md" | "mdx" => ("\u{e609} ", Color::Rgb(83, 141, 213)), // markdown blue
        "txt" => ("\u{e64e} ", Color::White),
        "pdf" => ("\u{e67d} ", Color::Rgb(210, 46, 46)), // pdf red
        "tex" | "latex" => ("\u{e69b} ", Color::Green),
        "rst" => ("\u{e64e} ", Color::DarkGray),

        // ── DevOps ──
        "tf" | "tfvars" => ("\u{e69a} ", Color::Rgb(92, 78, 229)), // terraform purple
        "nix" => ("\u{f1105} ", Color::Rgb(126, 186, 228)),        // nix blue

        // ── Media: images ──
        "png" | "jpg" | "jpeg" | "gif" | "bmp" | "webp" | "tiff" | "tif" => {
            ("\u{e60d} ", Color::Rgb(160, 100, 200))
        } // image purple
        ...
        "mp3" | "wav" | "flac" | "ogg" | "aac" | "m4a" | "wma" => {
            ("\u{e638} ", Color::Rgb(160, 100, 200))
        } // audio purple

        // ── Media: video ──
        "mp4" | "avi" | "mkv" | "mov" | "webm" | "wmv" | "flv" => {
            ("\u{e69f} ", Color::Rgb(224, 148, 64))
        } // video orange

        // ── Media: fonts ──
        "ttf" | "otf" | "woff" | "woff2" | "eot" => ("\u{e659} ", Color::Rgb(210, 46, 46)), // font red
        ...
        // ── Certificates / keys ──
        "pem" | "crt" | "cer" | "key" | "p12" | "pfx" => ("\u{f084} ", Color::Yellow),

        // ── Databases ──
        "db" | "sqlite" | "sqlite3" => ("\u{e64d} ", Color::Rgb(85, 141, 200)),

        // ── Binaries / executables ──
        "wasm" => ("\u{e6a1} ", Color::Rgb(101, 79, 240)), // wasm purple
        "o" | "so" | "dylib" | "dll" | "exe" => ("\u{eae8} ", Color::DarkGray), // binary
